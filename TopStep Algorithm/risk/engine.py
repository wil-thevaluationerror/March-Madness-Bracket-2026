...
class RiskEngine:
    def __init__(self, limits: RiskLimits, mode: str = "live") -> None:
        self.limits = limits
        self.mode = mode
    ...
    def can_trade(self, now: datetime) -> tuple[bool, str]:
        ...
        if not self.is_backtest_mode and self.state.locked:
            return False, "daily_lockout"
        if not self.is_backtest_mode and self.state.cooldown_until and now < self.state.cooldown_until:
            return False, "cooldown_active"
        if not self.is_backtest_mode and self.state.trade_count >= self.limits.max_trades_per_day:
            return False, "max_trades_reached"
        if not self.is_backtest_mode and self.state.daily_realized_pnl <= -abs(self.limits.internal_daily_loss_limit):
    ...
    def record_trade_close(self, pnl: float, now: datetime) -> None:
        ...
        if pnl < 0:
            ...
            if not self.is_backtest_mode and self.state.consecutive_losses >= self.limits.max_consecutive_losses:

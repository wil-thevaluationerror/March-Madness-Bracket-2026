...
from models.orders import BrokerOrder, ExecutionReport, OrderIntent, OrderState, OrderType, Side, TimeInForce
...
class SimulatedBacktestEngine:
    ...
    def run(self, bars: pd.DataFrame, intents: list[OrderIntent], now: datetime | None = None) -> BacktestResult:
        ...
        for position in open_positions:
            ...
            if last_bar is not None:
                ...
                result.trades.append(self._close_trade(position, last_bar["close"], last_bar["ts_event"], "end_of_data"))
        for simulation_state in concurrency_states.values():
    ...
    def _process_primary_exits(
        ...
        for position in positions:
            ...
            if exit_fill is not None and exit_price is not None and exit_reason is not None:
                self.execution_engine.drain_adapter_events()
                result.fills.append(exit_fill)
                result.trades.append(self._close_trade(position, exit_price, bar["ts_event"], exit_reason))
                continue
            remaining_positions.append(position)
    ...
    def _check_simulated_exit(
        ...
        if stop_hit:
            return None, position.stop_price, "stop_loss"
        if target_hit and position.target_price is not None:
            return None, position.target_price, "take_profit"
    ...
    def _check_exit(
        ...
        if stop_hit and position.stop_order_id is not None:
            ...
            return report, position.stop_price, "stop_loss"
        if target_hit and position.target_order_id is not None and position.target_price is not None:
            ...
            return report, position.target_price, "take_profit"
        if bar["ts_event"].time() >= time(15, 0):
    ...
    @staticmethod
    def _close_trade(position: SimulatedPosition, exit_price: float, exit_ts: datetime, exit_reason: str) -> SimulatedTrade:
        return SimulatedTrade(
            ...
            qty=position.qty,
            pnl=_trade_pnl(position.symbol, position.side, position.entry_price, exit_price, position.qty),

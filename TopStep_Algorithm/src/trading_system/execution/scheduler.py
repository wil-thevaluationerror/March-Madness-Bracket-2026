...
from trading_system.config import SessionConfig, SessionWindow
...
class SessionScheduler:
    ...
    def is_holiday(self, dt: datetime) -> bool:
        """Return True if dt falls on a date in config.skip_dates (Chicago-local)."""
        skip = getattr(self.config, "skip_dates", ())
        if not skip:
            return False
        local_date = self.localize(dt).date()
        return local_date in skip
